...
import type { Company } from "@/lib/constant";
          ...
          <div className="lowercase">
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-neutral-900 border-dashed">
              ...
              {isMobile ? (
                ...
                <motion.span
                  layoutId={`company-duration-${index}`}
                  transition={{
                    type: "spring",
                    stiffness: 300,
                    damping: 30,
            ...
            {/* Preview Carousel */}
            {company.preview && company.preview.length > 0 && (
              <motion.div
                initial={{ opacity: 0, filter: "blur(8px)", y: 8 }}
                animate={{ opacity: 1, filter: "blur(0px)", y: 0 }}
                exit={{ opacity: 0, filter: "blur(8px)", y: 8 }}
                    ...
                    {company.preview.map((img, idx) => (
                      <CarouselItem key={img} className="pl-0 basis-full">
                        <div className="relative aspect-video overflow-hidden rounded border border-[#1e1e1e]">
                          <Image
                            src={img}
                            alt={`${company.name} preview ${idx + 1}`}
                            fill
